...
def chinese_to_arabic(cn: str) -> int:
    ...
    for cndig in reversed(cn):
        # 避免“一万零千”、“二千零十二”等情况出现
        if cndig in ["零", "〇"] and last_is_unit:
            return np.nan
        if cndig in cn_unit:
            # 避免 "一千二千"、“一万万”等相同单位出现情况
            if cndig == last_unit:
                return np.nan
            # 避免"一万千"等单位重复情况出现
            if last_is_unit and last_unit not in ["万", "萬", "亿", "億", "兆"]:
                    return np.nan
            last_is_unit = True
            last_is_num = False
            last_unit = cndig
            unit = cn_unit.get(cndig)
            if unit == 10000 or unit == 100000000 or unit == 1000000000000:
                ldig.append(unit)
                unit = 1
        else:
            # 防止出现“七八”、“三五百”等数字重复出现情况
    ...
    for x in reversed(ldig):
        if x == 10000 or x == 100000000 or x == 1000000000000:
            val += tmp * x
            tmp = 0
        else:
            tmp += x
...
re_p_obj1, re_p_obj2, re_p_obj3 = re_compile2()
...
def robust_clean_percent(num) -> str:
    ...
    if re_p_obj3.search(num):
        return ''
    # 忽略数字之后跟随非有效词缀的词/字，如“100号”，“一般”
    if re_p_obj2.search(num):
        return ''
    result = re_p_obj1.sub('', num)
    # 如果转换后的值大于100，返回空值
    if result.isdigit():
        if int(result) > 100:
            return ''
