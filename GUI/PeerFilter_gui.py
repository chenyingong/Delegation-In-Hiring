...
def preprocess_files(path1: str, path2: str, path3: str) -> list:
    ...
    df_database = pd.DataFrame(
        df_database, columns=["工号", "姓名", "岗位", "入职日期", "出勤", "点号合计", "销售业绩", "工资年月", "部门", "区域"])
    df_database.fillna({"姓名": '-', "岗位": '-', "部门": '-', "区域": '-', "入职日期": '-', "工资年月": '-'}, inplace=True)
    df_database[["姓名", "岗位", "部门", "区域"]] = df_database[["姓名", "岗位", "部门", "区域"]].\
    ...
    # 员工信息表的内容在维护表更新时就载入了
    df_perform = pd.merge(df_perform, df_info, left_on="工号", right_on="员工号", how="inner")
    df_perform = df_perform.drop(["店名", "工号", "姓名_x", "岗位"], axis=1)
    df_perform = df_perform.rename(columns={'姓名_y': '姓名'})
    ...
    df_perform = df_perform.groupby("员工号")[["点号合计", "销售业绩"]].sum()  # sum the worker's performance
    df_perform.reset_index(drop=False, inplace=True)
    df_perform = pd.merge(df_perform, df_info, on="员工号", how="inner")

    # generate random numbers for the workers in df_perform
    df_perform["随机数"] = ralib.helper.sample_floats(0, 1, len(df_perform))
    global region
    region = df_perform["二级部门"].unique()
...
def peer_to_excel(df_perform: pd.DataFrame):
    ...
    for r in tqdm(region):
        ...
        for g in range(3):
            df = df_copy[(df_copy["二级部门"] == r) & (df_copy["群体"] == (g + 1))]
            df1 = df[df["岗位名称"] == "理疗师"]
            df2 = df[(df["岗位名称"] == "SPA师") | (df["岗位名称"] == "专家")]
            ...
            # difference list
            df1_unselected_id = set(df1["员工号"]).difference(set(df1_selected["员工号"]))
            df2_unselected_id = set(df2["员工号"]).difference(set(df2_selected["员工号"]))
            # stealing
            if (len1 < 5) and (len(df2_unselected_id) > 0):
                n = 5 - len1
                # if n > len(df2_unselected_id), iloc func can still work, and choose full range
                df1_selected = pd.concat([df1_selected, df2[df2["员工号"].isin(df2_unselected_id)].
                                         sort_values(by="随机数").
                                         iloc[:n]], ignore_index=True)
            elif (len(df1_unselected_id) > 0) and (len2 < 10):
                n = 10 - len2
                df2_selected = pd.concat([df2_selected, df1[df1["员工号"].isin(df1_unselected_id)].
                                         sort_values(by="随机数").
                                         iloc[:n]], ignore_index=True)
            ...
            df_result = df_result.reindex(["员工号", "岗位名称", "姓名", "点号合计", "销售业绩",
                                           "二级部门", "部门", "入职日期", "随机数", "群体"], axis=1)
            num_of_workers[g][r] = len(df_result)
            df_result.to_excel(writer, sheet_name=group_name[g], index=None)

        # group 4
        df = df_copy[(df_copy["二级部门"] == r) & (df_copy["群体"] == 4)]
        df = df[(df["岗位名称"] == "理疗师") | (df["岗位名称"] == "SPA师") | (df["岗位名称"] == "专家")]
        # sort by performance: descending
        ...
        df = df.reindex(["员工号", "岗位名称", "姓名", "点号合计",
                         "销售业绩", "二级部门", "部门", "入职日期", "随机数", "群体"], axis=1)
...
def senior_to_excel(df_perform: pd.DataFrame, df_database: pd.DataFrame):
    ...
    print("写入老人组数据:")
    for r in tqdm(region):
        df_by_region = df_perform_copy[df_perform_copy["二级部门"] == r]
        ...
        df_senior_long = df_database_copy[df_database_copy["工号"].isin(df_senior_id["员工号"])]
        ...
        df_senior_long["入职日期"] = df_senior_long["入职日期"].map(ralib.time.to_date)  # first trans to string type
        df_senior_long["工资年月"] = df_senior_long["工资年月"].map(ralib.time.to_date)
        # ??? corner case: rehire/more than one employment date
        df_senior_long["第n月工作"] = df_senior_long.apply(lambda row:
                                                       ralib.time.month_delta(row["入职日期"], row["工资年月"]), axis=1)
        df_senior_long = df_senior_long.reindex(["工号", "姓名", "区域", "部门", "岗位", "出勤", "点号合计", "销售业绩",
                                                 "工资年月", "入职日期", "第n月工作"], axis=1)
        df_senior_long.reset_index(drop=True, inplace=True)
        ...
        df_senior = pd.merge(df_senior, df_id_15, left_on="员工号", right_on="工号", how="inner")
        df_senior.sort_values("随机数", ascending=False, inplace=True)
        del df_senior["工号"]
        df_senior = df_senior.reindex(["员工号", "岗位名称", "姓名", "点号合计", "销售业绩",
                                       "二级部门", "部门", "入职日期", "随机数"], axis=1)
